...
The commands above will build the [amalgamation](https://www.sqlite.org/amalgamation.html) and the [Command Line Interface](https://www.sqlite.org/cli.html) (aka: [CLI](https://www.sqlite.org/cli.html)). There are no (required) dependencies, so the commands above should work as long as you have a compiler installed. No third-party software is needed.

For more detail see:
- [https://sqlite.org/src/doc/trunk/doc/compile-for-unix.md](https://sqlite.org/src/doc/trunk/doc/compile-for-unix.md)
- [https://sqlite.org/src/doc/trunk/doc/compile-for-windows.md](https://sqlite.org/src/doc/trunk/doc/compile-for-windows.md)
...
A build of the [command-line interface](https://www.sqlite.org/cli.html) requires three source files:
...
All three of the above source files are contained in the [amalgamation tarball](https://www.sqlite.org/download.html#amalgtarball) available on the [download page](https://www.sqlite.org/download.html). Or they can be built from [canonical sources](https://www.sqlite.org/amalg-v-canon.html#canon) using the "sqlite3.c" makefile target.
...
The -ldl library is needed to support dynamic loading, the [sqlite3_load_extension()](https://www.sqlite.org/c3ref/load_extension.html) interface and the [load_extension() SQL function](https://www.sqlite.org/lang_corefunc.html#load_extension). If these features are not required, then they can be omitted using [SQLITE_OMIT_LOAD_EXTENSION](https://www.sqlite.org/compile.html#omit_load_extension) compile-time option:

```
gcc -DSQLITE_THREADSAFE=0 -DSQLITE_OMIT_LOAD_EXTENSION shell.c sqlite3.c -lm -o sqlite3
```

One might want to provide other [compile-time options](https://www.sqlite.org/compile.html) such as:
- [-DSQLITE_ENABLE_FTS4](https://www.sqlite.org/compile.html#enable_fts4) or [-DSQLITE_ENABLE_FTS5](https://www.sqlite.org/compile.html#enable_fts5) for full-text search,
- [-DSQLITE_ENABLE_RTREE](https://www.sqlite.org/compile.html#enable_rtree) for the R*Tree search engine extension,
- [-DSQLITE_ENABLE_DBSTAT_VTAB](https://www.sqlite.org/compile.html#enable_dbstat_vtab) for the [dbstat virtual table](https://www.sqlite.org/dbstat.html), or
- [-DSQLITE_ENABLE_MATH_FUNCTIONS](https://www.sqlite.org/compile.html#enable_math_functions) for extended math functions.

In order to see extra commentary in [EXPLAIN](https://www.sqlite.org/lang_explain.html) listings, add the [-DSQLITE_ENABLE_EXPLAIN_COMMENTS](https://www.sqlite.org/compile.html#enable_explain_comments) option. Add -DHAVE_READLINE and the -lreadline and -lncurses libraries to get command-line editing support. One might also want to specify some compiler optimization switches. (The precompiled CLI available for download from the SQLite website uses "-Os".) There are countless possible variations here. A command to compile a full-featured shell might look something like this:
...
The versions of the SQLite amalgamation that are supplied on the [download page](https://www.sqlite.org/download.html) are normally adequate for most users. However, some projects may want or need to build their own amalgamations. A common reason for building a custom amalgamation is in order to use certain [compile-time options](https://www.sqlite.org/compile.html) to customize the SQLite library. Recall that the SQLite amalgamation contains a lot of C-code that is generated by auxiliary programs and scripts. Many of the compile-time options affect this generated code and must be supplied to the code generators before the amalgamation is assembled.

To build a custom amalgamation, first download the original individual source files onto a Unix or Unix-like development platform. Be sure to get the original source files, not the "preprocessed source files". One can obtain the complete set of original source files either from the [download page](https://www.sqlite.org/download.html) or directly from the [configuration management system](https://sqlite.org/src).
...
In most cases, you will want to supplement the basic commands above with [compile-time options](https://www.sqlite.org/compile.html) appropriate for your application. Commonly used compile-time options include:
...
- `-DSQLITE_ENABLE_FTS4` - Include the [full-text search](https://www.sqlite.org/fts3.html) engine code in SQLite.
- `-DSQLITE_ENABLE_RTREE` - Include the [R-Tree extension](https://www.sqlite.org/rtree.html).
