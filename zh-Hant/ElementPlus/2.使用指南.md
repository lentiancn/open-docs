# Element Plus 使用指南

本指南介紹如何在 Vue 3 應用中使用 Element Plus 組件。

## 快速開始

### 基礎範本

```vue
<template>
  <div>
    <el-button type="primary">主要按鈕</el-button>
    <el-input v-model="input" placeholder="輸入文字" />
    <el-date-picker v-model="date" type="date" placeholder="選擇日期" />
  </div>
</template>

<script setup>
import { ref } from 'vue'

const input = ref('')
const date = ref('')
</script>
```

## 常用組件

### 按鈕

```vue
<template>
  <!-- 類型 -->
  <el-button>預設</el-button>
  <el-button type="primary">主要</el-button>
  <el-button type="success">成功</el-button>
  <el-button type="warning">警告</el-button>
  <el-button type="danger">危險</el-button>
  <el-button type="info">資訊</el-button>

  <!-- 樸素按鈕 -->
  <el-button plain>樸素</el-button>
  <el-button type="primary" plain>主要樸素</el-button>

  <!-- 圓角按鈕 -->
  <el-button round>圓角</el-button>
  <el-button type="primary" round>主要圓角</el-button>

  <!-- 圓形按鈕 -->
  <el-button circle icon="Plus"></el-button>
  <el-button circle icon="Search"></el-button>

  <!-- 尺寸 -->
  <el-button size="large">大型</el-button>
  <el-button size="default">預設</el-button>
  <el-button size="small">小型</el-button>

  <!-- 停用 -->
  <el-button disabled>停用</el-button>
  <el-button type="primary" disabled>主要停用</el-button>
</template>
```

### 表單

```vue
<template>
  <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
    <el-form-item label="活動名稱" prop="name">
      <el-input v-model="form.name"></el-input>
    </el-form-item>
    
    <el-form-item label="活動區域" prop="region">
      <el-select v-model="form.region" placeholder="請選擇">
        <el-option label="區域一" value="shanghai"></el-option>
        <el-option label="區域二" value="beijing"></el-option>
      </el-select>
    </el-form-item>
    
    <el-form-item label="活動時間">
      <el-date-picker
        v-model="form.date"
        type="datetimerange"
        range-separator="至"
        start-placeholder="開始日期"
        end-placeholder="結束日期"
      ></el-date-picker>
    </el-form-item>
    
    <el-form-item label="即時配送">
      <el-switch v-model="form.delivery"></el-switch>
    </el-form-item>
    
    <el-form-item label="活動資源">
      <el-radio-group v-model="form.resource">
        <el-radio label="Sponsor">贊助商</el-radio>
        <el-radio label="Venue">場地</el-radio>
      </el-radio-group>
    </el-form-item>
    
    <el-form-item label="活動描述">
      <el-input v-model="form.desc" type="textarea"></el-input>
    </el-form-item>
    
    <el-form-item>
      <el-button type="primary" @click="onSubmit">建立</el-button>
      <el-button @click="onReset">重設</el-button>
    </el-form-item>
  </el-form>
</template>

<script setup>
import { reactive, ref } from 'vue'

const formRef = ref(null)

const form = reactive({
  name: '',
  region: '',
  date: [],
  delivery: false,
  resource: '',
  desc: ''
})

const rules = {
  name: [
    { required: true, message: '請輸入活動名稱', trigger: 'blur' },
    { min: 3, max: 5, message: '長度在 3 到 5 個字元', trigger: 'blur' }
  ],
  region: [
    { required: true, message: '請選擇活動區域', trigger: 'change' }
  ]
}

const onSubmit = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      console.log('提交!')
    }
  })
}

const onReset = () => {
  formRef.value.resetFields()
}
</script>
```

### 表格

```vue
<template>
  <el-table :data="tableData" stripe border>
    <el-table-column prop="date" label="日期" width="180"></el-table-column>
    <el-table-column prop="name" label="姓名" width="180"></el-table-column>
    <el-table-column prop="address" label="地址"></el-table-column>
    <el-table-column label="操作">
      <template #default="scope">
        <el-button size="small" @click="handleEdit(scope.$index, scope.row)">編輯</el-button>
        <el-button size="small" type="danger" @click="handleDelete(scope.$index, scope.row)">刪除</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>

<script setup>
const tableData = [
  { date: '2016-05-03', name: 'Tom', address: '上海市普陀區金沙江路 1899 號' },
  { date: '2016-05-02', name: 'Tom', address: '上海市普陀區金沙江路 1899 號' },
  { date: '2016-05-04', name: 'Tom', address: '上海市普陀區金沙江路 1899 號' },
  { date: '2016-05-01', name: 'Tom', address: '上海市普陀區金沙江路 1899 號' }
]

const handleEdit = (index, row) => {
  console.log(index, row)
}

const handleDelete = (index, row) => {
  console.log(index, row)
}
</script>
```

### 對話框

```vue
<template>
  <el-button type="text" @click="dialogVisible = true">開啟對話框</el-button>

  <el-dialog v-model="dialogVisible" title="提示" width="30%">
    <span>這是一條訊息</span>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click="dialogVisible = false">確定</el-button>
      </span>
    </template>
  </el-dialog>
</template>

<script setup>
import { ref } from 'vue'

const dialogVisible = ref(false)
</script>
```

### 導航選單

```vue
<template>
  <el-menu
    :default-active="activeIndex"
    class="el-menu-vertical"
    @select="handleSelect"
  >
    <el-sub-menu index="1">
      <template #title>
        <el-icon><Location /></el-icon>
        <span>導航一</span>
      </template>
      <el-menu-item-group title="群組一">
        <el-menu-item index="1-1">選項一</el-menu-item>
        <el-menu-item index="1-2">選項二</el-menu-item>
      </el-menu-item-group>
      <el-sub-menu index="1-4">
        <template #title>選項四</template>
        <el-menu-item index="1-4-1">選項一</el-menu-item>
      </el-sub-menu>
    </el-sub-menu>
    <el-menu-item index="2">
      <el-icon><Document /></el-icon>
      <span>導航二</span>
    </el-menu-item>
    <el-menu-item index="3">
      <el-icon><Setting /></el-icon>
      <span>導航三</span>
    </el-menu-item>
  </el-menu>
</template>

<script setup>
import { ref } from 'vue'
import { Location, Document, Setting } from '@element-plus/icons-vue'

const activeIndex = ref('1')
const handleSelect = (key, keyPath) => {
  console.log(key, keyPath)
}
</script>
```

### 卡片

```vue
<template>
  <el-row :gutter="20">
    <el-col :span="8" v-for="(item, index) in cards" :key="index">
      <el-card shadow="hover">
        <template #header>
          <div class="card-header">
            <span>{{ item.title }}</span>
            <el-button class="button" text>操作</el-button>
          </div>
        </template>
        <div v-for="o in 4" :key="o" class="text item">
          {{ '列表內容 ' + o }}
        </div>
      </el-card>
    </el-col>
  </el-row>
</template>

<script setup>
const cards = [
  { title: '卡片名稱 1' },
  { title: '卡片名稱 2' },
  { title: '卡片名稱 3' }
]
</script>

<style>
.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
}
.text {
  font-size: 14px;
}
.item {
  padding: 18px 0;
}
</style>
```

## 佈局

### 柵格系統

```vue
<template>
  <el-row :gutter="20">
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
  </el-row>
</template>

<style>
.el-col {
  border-radius: 4px;
}
.grid-content {
  border-radius: 4px;
  min-height: 36px;
  background: #409eff;
  color: white;
  text-align: center;
  padding: 20px;
}
</style>
```

### 間距

```vue
<template>
  <el-space wrap>
    <el-card v-for="i in 3" :key="i" :body-style="{ padding: '0px' }">
      <img src="/images/hamburger.png" class="image" />
      <div style="padding: 14px">
        <span>卡片名稱</span>
        <div class="bottom">
          <el-button text class="button">操作</el-button>
        </div>
      </div>
    </el-card>
  </el-space>
</template>
```

## 實用工具

### 圖示使用

```vue
<template>
  <!-- 使用 el-icon 組件 -->
  <el-icon :size="20" color="#409eff">
    <Plus />
  </el-icon>
  
  <!-- 在按鈕中使用 -->
  <el-button type="primary" :icon="Search">搜尋</el-button>
  <el-button type="primary">
    <el-icon><Plus /></el-icon>
    新增
  </el-button>
</template>

<script setup>
import { Plus, Search } from '@element-plus/icons-vue'
</script>
```

### 訊息提示

```vue
<script setup>
import { ElMessage } from 'element-plus'

const open = () => {
  ElMessage.success('成功訊息')
  ElMessage.warning('警告訊息')
  ElMessage.error('錯誤訊息')
  ElMessage.info('資訊訊息')
}
</script>
```

### 通知

```vue
<script setup>
import { ElNotification } from 'element-plus'

const open = () => {
  ElNotification({
    title: '成功',
    message: '這是一條成功訊息',
    type: 'success'
  })
}
</script>
```

## 最佳實踐

### 1. 使用組合式 API

```vue
<script setup>
import { ref, reactive } from 'vue'

// 使用 ref 處理原始值
const input = ref('')
const visible = ref(false)

// 使用 reactive 處理物件
const form = reactive({
  name: '',
  age: 0
})
</script>
```

### 2. 表單驗證

```vue
<script setup>
const rules = {
  name: [
    { required: true, message: '請輸入名稱', trigger: 'blur' },
    { min: 3, max: 10, message: '長度在 3 到 10 個字元', trigger: 'blur' }
  ],
  email: [
    { required: true, message: '請輸入電子郵件', trigger: 'blur' },
    { type: 'email', message: '請輸入有效的電子郵件地址', trigger: 'blur' }
  ],
  age: [
    { required: true, message: '請輸入年齡', trigger: 'blur' },
    { type: 'number', min: 18, message: '年齡必須大於 18', trigger: 'blur' }
  ]
}
</script>
```

### 3. 載入狀態

```vue
<template>
  <el-button v-loading="loading">載入</el-button>
  
  <el-table v-loading="loading" :data="data">
    <!-- 表格欄位 -->
  </el-table>
</template>

<script setup>
import { ref } from 'vue'

const loading = ref(false)
const data = ref([])

const loadData = async () => {
  loading.value = true
  try {
    data.value = await fetchData()
  } finally {
    loading.value = false
  }
}
</script>
```

## 常見問題

### 組件不渲染？

請確保組件已正確引入或全域註冊。

### 樣式不生效？

檢查是否已引入 Element Plus 樣式：

```javascript
import 'element-plus/dist/index.css'
```

### 圖示不顯示？

全域註冊圖示：

```javascript
import * as icons from '@element-plus/icons-vue'
for (const [key, icon] of Object.entries(icons)) {
    app.component(key, icon)
}
```

## 資源

- [官方文檔](https://element-plus.org/)
- [組件文檔](https://element-plus.org/zh-TW/component/button.html)
- [圖示文檔](https://element-plus.org/zh-TW/component/icon.html)
