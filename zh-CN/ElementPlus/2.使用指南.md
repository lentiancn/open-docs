# Element Plus 使用指南

本指南介绍如何在 Vue 3 应用中使用 Element Plus 组件。

## 快速开始

### 基础模板

```vue
<template>
  <div>
    <el-button type="primary">主要按钮</el-button>
    <el-input v-model="input" placeholder="输入文本" />
    <el-date-picker v-model="date" type="date" placeholder="选择日期" />
  </div>
</template>

<script setup>
import { ref } from 'vue'

const input = ref('')
const date = ref('')
</script>
```

## 常用组件

### 按钮

```vue
<template>
  <!-- 类型 -->
  <el-button>默认</el-button>
  <el-button type="primary">主要</el-button>
  <el-button type="success">成功</el-button>
  <el-button type="warning">警告</el-button>
  <el-button type="danger">危险</el-button>
  <el-button type="info">信息</el-button>

  <!-- 朴素按钮 -->
  <el-button plain>朴素</el-button>
  <el-button type="primary" plain>主要朴素</el-button>

  <!-- 圆角按钮 -->
  <el-button round>圆角</el-button>
  <el-button type="primary" round>主要圆角</el-button>

  <!-- 圆形按钮 -->
  <el-button circle icon="Plus"></el-button>
  <el-button circle icon="Search"></el-button>

  <!-- 尺寸 -->
  <el-button size="large">大型</el-button>
  <el-button size="default">默认</el-button>
  <el-button size="small">小型</el-button>

  <!-- 禁用 -->
  <el-button disabled>禁用</el-button>
  <el-button type="primary" disabled>主要禁用</el-button>
</template>
```

### 表单

```vue
<template>
  <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
    <el-form-item label="活动名称" prop="name">
      <el-input v-model="form.name"></el-input>
    </el-form-item>
    
    <el-form-item label="活动区域" prop="region">
      <el-select v-model="form.region" placeholder="请选择">
        <el-option label="区域一" value="shanghai"></el-option>
        <el-option label="区域二" value="beijing"></el-option>
      </el-select>
    </el-form-item>
    
    <el-form-item label="活动时间">
      <el-date-picker
        v-model="form.date"
        type="datetimerange"
        range-separator="至"
        start-placeholder="开始日期"
        end-placeholder="结束日期"
      ></el-date-picker>
    </el-form-item>
    
    <el-form-item label="即时配送">
      <el-switch v-model="form.delivery"></el-switch>
    </el-form-item>
    
    <el-form-item label="活动资源">
      <el-radio-group v-model="form.resource">
        <el-radio label="Sponsor">赞助商</el-radio>
        <el-radio label="Venue">场地</el-radio>
      </el-radio-group>
    </el-form-item>
    
    <el-form-item label="活动描述">
      <el-input v-model="form.desc" type="textarea"></el-input>
    </el-form-item>
    
    <el-form-item>
      <el-button type="primary" @click="onSubmit">创建</el-button>
      <el-button @click="onReset">重置</el-button>
    </el-form-item>
  </el-form>
</template>

<script setup>
import { reactive, ref } from 'vue'

const formRef = ref(null)

const form = reactive({
  name: '',
  region: '',
  date: [],
  delivery: false,
  resource: '',
  desc: ''
})

const rules = {
  name: [
    { required: true, message: '请输入活动名称', trigger: 'blur' },
    { min: 3, max: 5, message: '长度在 3 到 5 个字符', trigger: 'blur' }
  ],
  region: [
    { required: true, message: '请选择活动区域', trigger: 'change' }
  ]
}

const onSubmit = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      console.log('提交!')
    }
  })
}

const onReset = () => {
  formRef.value.resetFields()
}
</script>
```

### 表格

```vue
<template>
  <el-table :data="tableData" stripe border>
    <el-table-column prop="date" label="日期" width="180"></el-table-column>
    <el-table-column prop="name" label="姓名" width="180"></el-table-column>
    <el-table-column prop="address" label="地址"></el-table-column>
    <el-table-column label="操作">
      <template #default="scope">
        <el-button size="small" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
        <el-button size="small" type="danger" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>

<script setup>
const tableData = [
  { date: '2016-05-03', name: 'Tom', address: '上海市普陀区金沙江路 1899 号' },
  { date: '2016-05-02', name: 'Tom', address: '上海市普陀区金沙江路 1899 号' },
  { date: '2016-05-04', name: 'Tom', address: '上海市普陀区金沙江路 1899 号' },
  { date: '2016-05-01', name: 'Tom', address: '上海市普陀区金沙江路 1899 号' }
]

const handleEdit = (index, row) => {
  console.log(index, row)
}

const handleDelete = (index, row) => {
  console.log(index, row)
}
</script>
```

### 对话框

```vue
<template>
  <el-button type="text" @click="dialogVisible = true">打开对话框</el-button>

  <el-dialog v-model="dialogVisible" title="提示" width="30%">
    <span>这是一条消息</span>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click="dialogVisible = false">确定</el-button>
      </span>
    </template>
  </el-dialog>
</template>

<script setup>
import { ref } from 'vue'

const dialogVisible = ref(false)
</script>
```

### 导航菜单

```vue
<template>
  <el-menu
    :default-active="activeIndex"
    class="el-menu-vertical"
    @select="handleSelect"
  >
    <el-sub-menu index="1">
      <template #title>
        <el-icon><Location /></el-icon>
        <span>导航一</span>
      </template>
      <el-menu-item-group title="分组一">
        <el-menu-item index="1-1">选项一</el-menu-item>
        <el-menu-item index="1-2">选项二</el-menu-item>
      </el-menu-item-group>
      <el-sub-menu index="1-4">
        <template #title>选项四</template>
        <el-menu-item index="1-4-1">选项一</el-menu-item>
      </el-sub-menu>
    </el-sub-menu>
    <el-menu-item index="2">
      <el-icon><Document /></el-icon>
      <span>导航二</span>
    </el-menu-item>
    <el-menu-item index="3">
      <el-icon><Setting /></el-icon>
      <span>导航三</span>
    </el-menu-item>
  </el-menu>
</template>

<script setup>
import { ref } from 'vue'
import { Location, Document, Setting } from '@element-plus/icons-vue'

const activeIndex = ref('1')
const handleSelect = (key, keyPath) => {
  console.log(key, keyPath)
}
</script>
```

### 卡片

```vue
<template>
  <el-row :gutter="20">
    <el-col :span="8" v-for="(item, index) in cards" :key="index">
      <el-card shadow="hover">
        <template #header>
          <div class="card-header">
            <span>{{ item.title }}</span>
            <el-button class="button" text>操作</el-button>
          </div>
        </template>
        <div v-for="o in 4" :key="o" class="text item">
          {{ '列表内容 ' + o }}
        </div>
      </el-card>
    </el-col>
  </el-row>
</template>

<script setup>
const cards = [
  { title: '卡片名称 1' },
  { title: '卡片名称 2' },
  { title: '卡片名称 3' }
]
</script>

<style>
.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
}
.text {
  font-size: 14px;
}
.item {
  padding: 18px 0;
}
</style>
```

## 布局

### 栅格系统

```vue
<template>
  <el-row :gutter="20">
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
    <el-col :span="24" :md="8" :xs="24">
      <div class="grid-content">col-24 md-8 xs-24</div>
    </el-col>
  </el-row>
</template>

<style>
.el-col {
  border-radius: 4px;
}
.grid-content {
  border-radius: 4px;
  min-height: 36px;
  background: #409eff;
  color: white;
  text-align: center;
  padding: 20px;
}
</style>
```

### 间距

```vue
<template>
  <el-space wrap>
    <el-card v-for="i in 3" :key="i" :body-style="{ padding: '0px' }">
      <img src="/images/hamburger.png" class="image" />
      <div style="padding: 14px">
        <span>卡片名称</span>
        <div class="bottom">
          <el-button text class="button">操作</el-button>
        </div>
      </div>
    </el-card>
  </el-space>
</template>
```

## 实用工具

### 图标使用

```vue
<template>
  <!-- 使用 el-icon 组件 -->
  <el-icon :size="20" color="#409eff">
    <Plus />
  </el-icon>
  
  <!-- 在按钮中使用 -->
  <el-button type="primary" :icon="Search">搜索</el-button>
  <el-button type="primary">
    <el-icon><Plus /></el-icon>
    添加
  </el-button>
</template>

<script setup>
import { Plus, Search } from '@element-plus/icons-vue'
</script>
```

### 消息提示

```vue
<script setup>
import { ElMessage } from 'element-plus'

const open = () => {
  ElMessage.success('成功消息')
  ElMessage.warning('警告消息')
  ElMessage.error('错误消息')
  ElMessage.info('信息消息')
}
</script>
```

### 通知

```vue
<script setup>
import { ElNotification } from 'element-plus'

const open = () => {
  ElNotification({
    title: '成功',
    message: '这是一条成功消息',
    type: 'success'
  })
}
</script>
```

## 最佳实践

### 1. 使用组合式 API

```vue
<script setup>
import { ref, reactive } from 'vue'

// 使用 ref 处理原始值
const input = ref('')
const visible = ref(false)

// 使用 reactive 处理对象
const form = reactive({
  name: '',
  age: 0
})
</script>
```

### 2. 表单验证

```vue
<script setup>
const rules = {
  name: [
    { required: true, message: '请输入名称', trigger: 'blur' },
    { min: 3, max: 10, message: '长度在 3 到 10 个字符', trigger: 'blur' }
  ],
  email: [
    { required: true, message: '请输入邮箱', trigger: 'blur' },
    { type: 'email', message: '请输入有效的邮箱地址', trigger: 'blur' }
  ],
  age: [
    { required: true, message: '请输入年龄', trigger: 'blur' },
    { type: 'number', min: 18, message: '年龄必须大于 18', trigger: 'blur' }
  ]
}
</script>
```

### 3. 加载状态

```vue
<template>
  <el-button v-loading="loading">加载</el-button>
  
  <el-table v-loading="loading" :data="data">
    <!-- 表格列 -->
  </el-table>
</template>

<script setup>
import { ref } from 'vue'

const loading = ref(false)
const data = ref([])

const loadData = async () => {
  loading.value = true
  try {
    data.value = await fetchData()
  } finally {
    loading.value = false
  }
}
</script>
```

## 常见问题

### 组件不渲染？

请确保组件已正确引入或全局注册。

### 样式不生效？

检查是否已引入 Element Plus 样式：

```javascript
import 'element-plus/dist/index.css'
```

### 图标不显示？

全局注册图标：

```javascript
import * as icons from '@element-plus/icons-vue'
for (const [key, icon] of Object.entries(icons)) {
    app.component(key, icon)
}
```

## 资源

- [官方文档](https://element-plus.org/)
- [组件文档](https://element-plus.org/zh-CN/component/button.html)
- [图标文档](https://element-plus.org/zh-CN/component/icon.html)
